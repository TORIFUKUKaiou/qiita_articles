...
tags:
  ...
  - 闘魂
private: false
updated_at: '2024-12-15T23:47:19+09:00'
...
1. **Edit `/home/vscode/livebook/config/prod.exs`**  
   ...
   *(Note: I attempted to restrict this further by resolving the load balancer's hostname to an IP address via `nslookup`, but Phoenix failed to start.)*
   ...
   ![Screenshot 2024-12-15 11.02.26.png](https://qiita-image-store.s3.ap-northeast-1.amazonaws.com/0/131808/3dfc6b13-906b-b786-7a62-b77fec401958.png)

2. **Start Livebook in the Terminal**  
   Open the **Terminal** tab, add a new Bash session, and run the command `mix phx.server` to start Livebook. Note the token displayed in the logs; this token will serve as the password. *(Since the token changes on each startup, sharing it here publicly poses no security risks.)*
...
- **Manual Launch for Token Generation**  
   By manually running `mix phx.server`, you'll generate a unique `token` during the startup process, allowing you to access Livebook securely.
